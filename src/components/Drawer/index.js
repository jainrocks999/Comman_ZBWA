...
import { handleUnauthorizedError } from '../SessionExpire';
...
const Drawer = () => {
    ...
    const handleLogout = async () => {
        ...
        console.log('jgfdgjfdklg', user_token);
    ...
    const ManageCompanyProfile = async () => {
        navigation.dispatch(DrawerActions.closeDrawer()); // Drawer band karo
    ...
    useEffect(() => {
        const checkMemberStatus = async () => {
            try {
                let memberStatus = await AsyncStorage.getItem("isMember");

                console.log("Fetched from AsyncStorage:.................", memberStatus);
                setIsMember(memberStatus);

            } catch (error) {
                console.log("Error fetching member status:", error);
                setIsMember(null);
            }
        };
         ...
         <LinearGradient
            ...
            <ScrollView style={{ marginBottom: 50 }}>
                ...
                <View style={styles.view1}>
                    ...
                    {isMember == 1 && (
                        <TouchableOpacity onPress={ManageCompanyProfile}>
                            <Text style={styles.same}>Our Company Profile</Text>
                        </TouchableOpacity>
                    )}

                    <TouchableOpacity
                        onPress={() => ManageDirectory()}
                    >
            ...
            <View style={styles.bottom}>
                <Text style={styles.about}>Version <Text style={styles.about}>{version}</Text></Text>
            </View>
