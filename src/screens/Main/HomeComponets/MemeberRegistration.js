...
const MemeberRegistration = ({navigation}) => {
  ...
  const apiCall = async () => {
    ...
    axios
      .request(config)
      .then(response => {
        console.log('response ffddssdsd323', response.data.data.user);
        if (response.data.code == '200') {
          ...
          handleInputs('phone', response?.data?.data?.user?.phone || '');
          handleInputs(
            'companyName',
          ...
          <View style={{marginTop: 15}}>
            ...
            <View style={{flexDirection: 'row'}}>
              ...
              <Text
                ...
                {' '}
                Upload a valid Aadhar card to prevent registration cancellation.
              </Text>
            </View>
          </View>
        </View>
